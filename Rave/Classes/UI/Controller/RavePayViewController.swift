...
public class RavePayViewController: UITableViewController {
    ...
    var expandables = [Expandables(isExpanded: true, section: 0),Expandables(isExpanded: false, section: 1),Expandables(isExpanded: false, section: 2),
                       Expandables(isExpanded: false, section: 3),Expandables(isExpanded: false, section: 4)]
    ...
    func accountCallbacks(){
        ...
        raveAccountClient.banks = {(banks) in
            self.banks = banks
        }
        ...
        raveAccountClient.validateError = {[weak self](message,data) in
            ...
            if let _data  = data{
                ...
                if (message?.containsIgnoringCase(find: "Timed Out"))!{
                   self?.delegate?.tranasctionSuccessful(flwRef: nil)
                }else{
                    self?.delegate?.tranasctionFailed(flwRef: nil)
                }
            }
            self?.dismiss(animated: true)
        }
    ...
    func mpesaCallbacks(){
        ...
        raveMpesaClient.error = {(message,data) in
            LoadingHUD.shared().hide()
            DispatchQueue.main.async {
                if let msg = message{
                    if msg.containsIgnoringCase(find: "timeout"){
    ...
    func mobileMobileCallbacks(){
        ...
        raveMobileMoney.error = {(message,data) in
            LoadingHUD.shared().hide()
            DispatchQueue.main.async {
                if let msg = message{
                    if msg.containsIgnoringCase(find: "timeout"){
    ...
    func configureView(){
        ...
        headers = [nil,debitCardHeader,bankAccountHeader, mpesaHeader,ghanaMobileMoneyHeader]
    ...
    override public func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat {
        ...
        switch section {
            ...
            case 2:
                return RaveConfig.sharedConfig().country == "NG" ? 65 : 0
            case 3:
...
extension RavePayViewController : UITextFieldDelegate,RavePayWebProtocol,UIPickerViewDelegate,UIPickerViewDataSource,CardSelect {
    ...
    func handelCloseOrExpandSection(section:Int){
        ...
        if expandedState{
            ...
            //Expand current view and collapse the first section
